...
class ISACDatasetGenerator3GPP:
    ...
    def apply_channel(self, tx_signal, clusters):
        ...
        for delay, doppler_shift, _, power in clusters:
    ...
    def apply_uav_array_channel(self, tx_signal, range_m, velocity_mps, doa_deg, uav_size="small"):
        """Generate complex baseband signals across an 8-element ULA."""
    ...
    def apply_full_channel(self, tx_signal, range_m, velocity_mps, doa_deg, uav_size="small"):
        ...
        background_rx = self.apply_channel(
            tx_signal,
            self.generate_clusters(range_m, velocity_mps=0, doa_deg=0, is_target=False),
        )
    ...
    def generate_sample(self, range_m, velocity_mps, doa_deg, uav_size="small"):
        ...
        return tx, rx, np.array([range_m, velocity_mps, doa_deg], dtype=np.float32)
    ...
    def build_dataset(self, n_samples=5000, out_file="adyant_isac_dataset.h5"):
        ...
        for _ in range(n_samples):
            ...
            tx, rx, label = self.generate_sample(r, v, a, size)
            TX.append(tx)
            X.append(rx)
            y.append(label)
        ...
        print(f"Dataset generated: {path} | X={X.shape} | TX={TX.shape} | scenario={self.scenario}")


if __name__ == "__main__":
    scenarios = ["UMi", "UMa", "RMa", "SMa"]
    samples_per_scenario = int(os.getenv("TRINETRA_SAMPLES", "5000"))
    for sc in scenarios:
        gen = ISACDatasetGenerator3GPP(carrier_freq=6e9, scenario=sc, n_antennas=8)
        gen.build_dataset(n_samples=samples_per_scenario, out_file=f"adyant_isac_{sc}.h5")
