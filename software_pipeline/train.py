...
class UAVDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # X is now [antennas, samples]. The DOA input is estimated from
        # the antenna array instead of being taken from the ground-truth label.
        rx_array = self.X[idx]
        label = self.y[idx]

        # Coherently combine the array for the existing range/Doppler branches.
        rx_combined = np.mean(rx_array, axis=0)
        estimated_doa = self.fe.doa_from_ula(rx_array)
        feats = self.fe.extract_features(
            rx_combined,
        ...
        return (
            ...
            torch.tensor(estimated_doa / DOA_MAX, dtype=torch.float32),
        )
